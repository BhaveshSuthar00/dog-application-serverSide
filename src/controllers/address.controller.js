...
router.post("/post", async (req, res) => {
  try {
    ...
    if(address){
      return res.status(301).json({message : "user already has a home address"})
    }
...
router.get('/sort', async(req, res)=>{
  try {
    ...
    let data;
    if(query === 'asc'){
      data = await Address.find({}).sort({costPerDay : 1}).populate({path : "userId"}).lean().exec();
      ...
      data = await Address.find({verified : query}).populate({path : "userId"}).lean().exec();
      return res.status(200).json({Address : data});
    } else {
...
router.get("/:id", async (req, res) => {
  try {
    ...
    let residentData = await Resident.find({AddressId : req.params.id}).countDocuments();
    let resident = []
