...
router.post("/post", async (req, res) => {
  try {
    const data = await Data.findOne({addressId : req.body.addressId}).lean().exec();
    if(data){
...
router.patch('/reqtolist/:id', async (req, res) => {
  try {
    ...
    let redDd = await Data2.reqPets[query] !== undefined || null ? Data2.reqPets[query] : 'z';
    let pet = await Data2.Pets || [];
    if(redDd === 'z') {
      return res.status(200).json({message : "this pet already admitted"})
    }
    Data2 = {...Data2, reqPets : Data2.reqPets.filter((p)=> {
      return p !== redDd
    }), 
